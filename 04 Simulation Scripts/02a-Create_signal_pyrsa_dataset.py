...
def get_stim_desc(descriptors_path, label_dict = None):
    ...
    stim_ids = pd.read_csv(descriptors_path, sep='[, _]', header=None,
                           engine='python')
    stim_ids.columns = ['row', 'cond_', 'RegressorNames', 'run_', 'run']
...
label_dict_o     = np.load(os.path.join(ds_dir, "custom_synset_dictionary.npy"),
...
for sub in range(1, n_subs+1):     
    ...
    ds_output_dir = os.path.join(ds_dir, "derivatives", "PyRSA_GT", "datasets",
                                 "sub-"+str(sub).zfill(2))
    if not os.path.isdir(ds_output_dir):
        os.makedirs(ds_output_dir)
    res_output_dir = os.path.join(ds_dir, "derivatives", "PyRSA_GT", "noise",
                                  "sub-"+str(sub).zfill(2))
    ...
    signal_path = os.path.join(glm_dir, "sub-"+str(sub).zfill(2)+ "_" +
                               ses_type + "_signal.nii.gz")
    descriptors_path = os.path.join(glm_dir, "sub-"+str(sub).zfill(2)+ "_" +
                               ses_type + "_signal.csv")
    signal_4d = nifti1.load(signal_path)
    
    noise_4d = None
    noise_path = os.path.join(glm_dir, "sub-"+str(sub).zfill(2)+ "_" +
                              ses_type + "_noise.nii.gz")
