...
for roi_h in ['V1_left']:#roi_h_list:
    rdms = [] 
    ...
    for sub in range(1, n_subs+1): 
        ...
        # Load RDM
        rdm_filename = os.path.join(rdm_dir, "RDM_data_perm_mixed")
        rdm = pyrsa.rdm.rdms.load_rdm(rdm_filename, file_type='hdf5')
        roi_rdms = rdm.subset('prec_type', 'res-total').subset(
                        'snr_rel', 1.0).subset('perm', 1).subset(
                            'n_runs', 32).subset('roi', roi_h)
        # Collect single RDMs
        if isinstance(rdms, list):
            rdms = roi_rdms
        else:
            rdms.append(roi_rdms)
         
    # Plot RDMs for subject
    plot_rdm(rdms, dpi=1200, do_rank_transform=True, title=
