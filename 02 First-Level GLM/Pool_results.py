...
# -*- coding: utf-8 -*-
"""
Pipeline for pooling the results of SPM's GLM
...
def pooled_betas_pipeline(sub, spm_dir, spm_type, ses_type, first_n_betas = None):
    run_counter = 0
...
def pooled_residuals_pipeline(sub, spm_dir, spm_type, ses_type):
    run_counter = 0
    ...
    for ses in range(1, n_ses+1):      
        ...
        for run in range(1, n_runs+1):
            run_counter += 1
            glm_dir = os.path.join(run_dir, "run-"+str(run).zfill(2))
            n_res = len(glob.glob(os.path.join(spm_dir,
                                               "sub-"+ str(sub).zfill(2),
                                               ses_type + str(ses).zfill(2),
                                               "run-" + str(run).zfill(2),
                                               "Res_*")))
...
for sub in range(2, n_subs+1):     
    ...
    if processing_mode in ['datasets', 'both']:
        ...
        pooled_betas_array, generic_affine, fourth_dimension_descriptors = \
            pooled_betas_pipeline(sub, spm_dir, spm_type, ses_type,
                                  first_n_betas = first_n_betas)
    ...
    if processing_mode in ['residuals', 'both']:
        ...
        pooled_residuals_array, generic_affine_r, fourth_dimension_descriptors_r = \
            pooled_residuals_pipeline(sub, spm_dir, spm_type, ses_type)
