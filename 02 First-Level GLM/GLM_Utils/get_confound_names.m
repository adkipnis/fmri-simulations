function confound_names = get_confound_names(Opts)
    ...
    if ~strcmp(Opts.use_global_signals, 'none')
        ...
        if strcmp(Opts.use_standard_regressors, 'deriv') % add their first derivatives only
            confound_names = [confound_names, deriv_global_signals];
        elseif strcmp(Opts.use_standard_regressors, 'power') % add their powers only
            confound_names = [confound_names, power_global_signals];
        elseif strcmp(Opts.use_standard_regressors, 'full') % add their first derivatives, powers for both the basic and the derivative parameters
    ...
    if Opts.n_pcs > 0
        for k = 1:Opts.n_pcs 
            anatomical_PCs(k) = strcat('a_comp_cor_0', string(k-1)); %python indexing starts with 0
        end 
...
%     end
    ...
    if Opts.n_cos > 0
        if Opts.t_smooth == true
            ...
            for k = 1:Opts.n_cos
                cosine_basis_functions(k) = strcat('cosine0', string(k-1));
            end
