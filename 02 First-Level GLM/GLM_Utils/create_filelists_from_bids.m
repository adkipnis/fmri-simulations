function Dirs = create_filelists_from_bids(Dirs, i, s)
            ...
            nii_filt = logical(contains(Dirs.file_names_p, 'preproc').*contains(Dirs.file_names_p, 'T1w').*contains(Dirs.file_names_p, 'nii.gz')); %only list all files containing these substrings
            Dirs.nii_files = strcat(Dirs.file_locs_p(nii_filt), filesep, Dirs.file_names_p(nii_filt));
            Dirs.n_runs = length(Dirs.nii_files);
            nii_filt_mask = logical(contains(Dirs.file_names_p, 'mask').*contains(Dirs.file_names_p, 'T1w').*contains(Dirs.file_names_p, 'nii.gz')); %only list all files containing these substrings
